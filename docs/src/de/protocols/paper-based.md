...
* **Web-Anwendungen** für **Betreiber**, **Gesundheitsämter** und **Nutzer**.
...
Die Kontaktdaten sollen hierbei nur auf vertauenswürdigen System verarbeitet werden können und möglichst wenigen Akteuren im System bekannt sein (egal ob in verschlüsselter oder unverschlüsselter Form).

Um Kontaktdaten zu erfassen, öffnen Nutzer zunächst die **Zilp-Zalp** Web-Anwendung und erfassen dort relevante Daten wie Name, Anschrift, Telefonnummer und E-Mail Adresse (eine Validierung dieser Daten kann in einer Protokollerweiterung durch externe Dienste erfolgen  und wird in `v0.2` beschrieben). Anschließend generiert die Anwendung zwei zufallsgenerierte, symmetrische Schlüssel $K _ A$ und $K _ B$, die über ein geeignetes Schlüsselableitungsverfahren miteinander zu einem Schlüssel $K _ C$ kombiniert werden. Die Anwendung verschlüsselt nun die Kontaktdaten des Nutzers symmetrisch mit Schlüssel $K _ C$, fügt zu diesen verschlüsselten Daten den Schlüssel $K _ A$ hinzu, verschlüsselt diese Daten asymmetrisch mit dem öffentlichen Schlüssel der **GÄs** und übermittelt diese Daten an die **Zilp-Zalp API**, welche sie in einem Backend ablegt. Die dort abgelegten Daten sind für keinen Akteur ohne Kenntnis des Schlüssels $K _ B$ entschlüsselbar. Obgleich hier eine "zentrale" Speicherung vorliegt, kann eine Verarbeitung der gespeicherten Daten nur unter Zuhilfenahme des privaten Schlüssels der **GÄ** sowie von $K _ B$ erfolgen, letzterer befindet sich unter Kontrolle des Nutzers. Es ist auch vorstellbar, die Kontaktdaten nur lokal zu speichern oder in diesem Schritt noch keine Kontaktdaten anzugeben, und diese nur unter Mitwirkung des Nutzers später zu erstellen oder freizugeben.

Als Antwort auf die API-Anfrage liefert die **Zilp-Zalp API** dem Nutzer eine große Anzahl von Zufallswerten (z.B. 100). Diese Werte werden vom Backend errechnet, indem zunächst ein Zufallswert $H _ 1$ generiert wird und dieser anschließend mit einem nur dem Backend vorliegenden Schlüssels $K _ s$ wiederholt kombiniert wird um eine Reihe von Werten $H _ 2\cdots H _ n$ zu erzeugen. Das Backend speichert den Wert $H _ 1$ zusammen mit den verschlüsselten Daten ab. 

Die Anwendung des Nutzers generiert nun Werte $Z _ i = H _ {2i+1} \oplus K _ B$ und paar sie mit den Werten $H _ {2i+2}$ zu $(Z _ i, H _ {2i+2}) = (H _ {2i+1} \oplus K _ B, H _ {2i+2})$. Diese Wertepaare dienen zur Kontaktnachverfolgung und können mithilfe des Backends verschlüsselten Kontaktdaten zugeordnet werden. Andere Akteure wie Betreiber oder Nutzer können jedoch ohne Kenntnis von $K _ s $ Wertepaare nicht einzelnen Nutzern zuordnen. Gegebenenfalls kann das Backend jeden der Hashwerte $H _ i$ zusäzlich mit einer Signatur $S _ i$ versehen um dessen Herkunft zu belegen (`v0.2`).

Jedes Wertepaar $(Z _ i, H _ {2i+2})$ kann nun zu einem QR-Code kodiert werden, diese Codes können ausgedruckt und zur Dokumentation von Besuchen verwendet werden. Hierbei können für Nutzer eine große Zahl an QR-Codes generiert werden (z.B. 100). Einer der QR-Codes wird hierbei für die Übermittlung ans Gesundheitsamt vorbehalten, welches diesen Zur Kontaktnachverfolgung nutzen kann. Alle anderen werden für die Dokumentation von Besuchen verwendet.

### Besuchsdokumentation

Zur Dokumentation des Besuchs einer Ortschaft gegenüber Betreibern übergeben Nutzer diesen einfach jeweils einen zufälligen QR-Code. Hierbei ist es möglich, neben dem QR-Code zusätzliche Meta-Daten wie die genaue Ankunftszeit sowie die Verweildauer einzutragen, um die Genauigkeit der Dokumentation zu erhöhen. Der Betreiber erfasst die über einen gegebenen Zeitraum (z.B. einen Tag) erhaltenen QR-Codes anschließend mithilfe die Zilp-Zalp Web-Anwendung, sie werden lokal gespeichert. Betreiber können hierbei ebenfalls zusätzliche Meta-Daten erfassen um die Genauigkeit zur Kontaktnachverfolgung zu verbessern.

### Kontaktnachverfolgung

Um mögliche Risikokontakte eines infizierten Nutzerr zu ermitteln übergibt dieser zunächst einen seiner QR-Codes an das Gesundheitsamt (entweder digital oder analog). Dieses erstellt unter Zuhilfenahme des Backends eine signierte Anfrage, die alle Hashwerte $H _ {2i+2}$ des Nutzers beinhaltet. Die Web-Anwendungen der Betreiber laden regelmäßig die Liste dieser Anfragen herunter und gleichen sie mit den lokal gespeicherten Hashwerten ab. Ergibt sich eine Übereinstimmung, werden nach Bestätigung durch den Betreiber alle Besuchsdaten die mit diesen in Zusammenhang stehen (z.B. ermittelt durch Vergleich der Besuchszeiten) an die Zilp-Zalp API übertragen. Diese sucht die diesen entsprechenden, zweifach verschlüsselten Kontaktdaten heraus und stellt sie zusammen mit den Besuchsdaten dem Gesundheitsamt zur Verfügung. Dieses entfern zunächst mit seinem privaten Schlüssel die äußere Verschlüsselung und nutzt dann den innen vorliegenden Schlüssel $K _ A$, sowie den aus den Besuchsdaten mithilfe von $K _ s$ rekonstruierten Schlüssel $K _ B$ um wiederum $K _ C$ zu rekonstruieren, womit anschließend die Kontaktdaten entschlüsselt werden können.
...
* Eine Kompromittierung des Backends macht einem Angreifer lediglich wenige, relativ unkritische Meta-Daten zugänglich. Das Backend speichert lediglich verschlüsselte Kontaktdaten und Hash-Werte, nicht jedoch wie in anderen Systemen komplette Besuchshistorien.
* Die Datenhaltung im Backend sowie der Kommunikationsaufwand ist sehr überschaubar. Die Dokumentation von Besuchen erfolgt dezentral und ohne Kommunikation mit dem Backend, es fallen daher bei Besuchen keine Meta-Daten ab.
...
* Für die Dokumentation eines Besuchs ist keine Interaktion eines Nutzers mit einer Web-Anwendung oder einem Smartphone notwendig. Betreiber können Besuche auch asynchron erfassen und mit zusätzlichen Metadaten versehen. Der Aufwand zum Einlesen von QR-Codes ist gering, und die Aufbewahrung der Codes durch den Betreiber stellt ein zusätzliches Backup zu den digitalen Daten dar.
* QR-Codes müssen nicht in Form von Schlüsselanhängern bezogen werden sondern können selbst ausgedruckt werden. Betreiber können theoretisch auch vorgefertigte QR-Code Serien an Nutzer ausgeben, welche diese dann im Rahmen der Protokollerweiterung unten selbst mit ihren Daten verknüpfen.
...
Dieser Angriff ist jedoch bei der Kontaktnachverfolung leicht erkennbar, die durch Mehrfachverwendung eines QR-Codes generierten Daten können vermutlich leicht entfernt werden. Mögliche Quellen des Missbrauchs lassen sich zudem anhand vorliegender sowie fehlender Daten mithilfe einer Befragung des Nutzers einfach rekonstruieren, fehlende QR-Codes können bei einer Überprüfung eines Betreibers anschließend identifziert werden. Eine missbräuchliche Nutzung ist damit (wie bei anderen Verfahren) nicht ausgeschlossen, kann aber im Gegensatz zu diesen effektiv nachverfolgt und geahndet werden.

#### Hinterlegung falscher Daten

Wie bei anderen Systemen ist die Validierung von Nutzerdaten bei gleichzeitiger Einhaltung der Pseudonymität eines Nutzers schwierig. Die folgende Version (`0.2`) dieses Protokolls soll daher eine Möglichkeit zur Validierung einzelner Kontaktdaten beinhalten. Generell ist eine durchgängige Validierung jedoch kaum möglich, ohne ein sehr invasives Überwachungssystem zu schaffen. Generell basiert die Kontaktnachverfolgung auf dem Eigeninteresse und der Eigenverantwortung der Beteiligten und selbst die Gesetzgebung hat hierbei anerkannt, dass eine zwangsweise Validierung nicht sinnvoll ist.

#### Rekonstruktion von Besuchshistorien

Die **Zilp-Zalp API** verfügt über alle Informationen die nötig sind, um alle Hashwerte die einem Nutzer zugeordnet sind zu rekonstruieren. Dies ist im Rahmen des Protokolls notwendig, um die Besuchshistorie eines Nutzers mithilfe der Betreiber zu rekonstruieren. Würde ein Dritter sowohl in den Besitz der im Backend gespeicherten Werte $H _ 1$ als auch des Schlüssels $ K _ s$ gelangen, könnte er eventuell beobachtete einzelne Besuchsdaten spezifischen Nutzern zuzuorden. Hierzu wäre jedoch weiterhin erforderlich, dass er ebenfalls Zugang zu diesen Daten erhält, welche dezentral bei den Betreibern gespeichert werden.

### Verbesserungsmöglichkeiten

Die folgenden Aspekte des Protokolls sind aus unserer Sicht noch verbesserungswürdig:
...
* Die Web-Anwendung des Nutzers erkennt anhand des veröffentlichten Zufallswert $Z _ 1$, dass die Daten des Nutzers zur Kontaktnachverfolgung angefordert wurden. Es fordert diesen dann dazu auf, die Daten anzugeben, verschlüsselt diese wie im normalen Protokollablauf und teilt sie über die API dem Backend mit, wobei der Zufallswert $ Z _ 2 $ als Validierung dient (um zu verhindern, dass andere Nutzer die Daten vervollständigen können).
* Das Backend kann nun die Daten an das Gesundheitsamt weitergeben.
